...
namespace svg {
    ...
    // --- PathProps ------------------------------------------------------------------
    // Базовый класс для работы с цветом (и другими параметрами) объектов Circle, Polyline, Text,
    // шаблонный, чтобы возвращать владельца, чтобы поддерживать method chaining (a.f1().f2()...)
    ...
    // --- Object Container ----------------------------------------------------------
    // Абстрактный класс-интерфейс для подключения различных объектов
    class ObjectContainer {
    public:
        virtual ~ObjectContainer() = default;

        // шаблоны не могут быть виртуальными, поэтому определяется здесь
    ...
    public:
        ...
        // (не нужен здесь, т.к появляется ObjectContainer)
        /*template <typename Obj>
        void Add(Obj obj);//*/
